...
for opcode, data in opcodes["cbprefixed"].items():
    ...
    if data["mnemonic"] == "BIT":
        rust_code += f"            if get_bit_at_position_{16 if 'HL' in [operand['name'] for operand in data['operands']] else 8}bit(3, gb.cpu.get_{[operand['name'] for operand in data['operands']][1].lower()}()) == 1 {{\n"
